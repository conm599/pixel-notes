...
(function (window) {
  ...
  function renderLines(lines) {
    ...
    while (i < n) {
      ...
      if (fence) {
        ...
        i++; // 跳过闭合围栏
        // mermaid 图表块：输出 div.mermaid 交给 mermaid 库渲染（库未加载时按代码块样式显示源码）
        if ((fence[2] || '').toLowerCase() === 'mermaid') {
          html.push('<div class="mermaid">' + escapeHtml(buf.join('\n')) + '</div>');
          continue;
        }
  ...
  window.PixelMD = { render: render, escapeHtml: escapeHtml, runMermaid: runMermaid };

  /* === mermaid 图表渲染：按需加载 CDN（页面有 div.mermaid 才注入 ~1MB 脚本，无图页面零开销） === */
  var MERMAID_CDN = 'https://cdn.jsdelivr.net/npm/mermaid@10.9.3/dist/mermaid.min.js';
  function ensureMermaidInit() {
    if (!window.mermaid || window.mermaid._pnInit) return;
    window.mermaid._pnInit = true;
    try {
      window.mermaid.initialize({
        startOnLoad: false,
        theme: 'dark',
        fontFamily: 'Consolas, "Courier New", monospace',
        securityLevel: 'strict' // 图内文本按纯文本处理，防 HTML 注入
      });
    } catch (e) { /* 初始化失败按未启用处理 */ }
  }
  function loadMermaid(ready) {
    if (window.mermaid) { ready(); return; }
    loadMermaid._cbs = loadMermaid._cbs || [];
    loadMermaid._cbs.push(ready);
    if (loadMermaid._started) return;
    loadMermaid._started = true;
    var s = document.createElement('script');
    s.src = MERMAID_CDN;
    s.async = true;
    s.onload = function () {
      ensureMermaidInit();
      var cbs = loadMermaid._cbs; loadMermaid._cbs = [];
      cbs.forEach(function (f) { f(); });
    };
    s.onerror = function () { loadMermaid._started = false; loadMermaid._cbs = []; };
    document.head.appendChild(s);
  }
  function runMermaid(root) {
    var scope = root && root.querySelectorAll ? root : document;
    var nodes = scope.querySelectorAll('div.mermaid:not([data-mermaid-done])');
    if (!nodes.length) return;
    // 库未就绪：动态注入 CDN，就绪后重跑本函数
    loadMermaid(function () { runMermaid(root); });
    if (!window.mermaid || !window.mermaid.run) return;
    var arr = Array.prototype.slice.call(nodes);
    arr.forEach(function (n) { n.setAttribute('data-mermaid-done', '1'); }); // 先标记防重入
    try {
      window.mermaid.run({ nodes: arr }).catch(function () {
        arr.forEach(function (n) { n.classList.add('mermaid-err'); }); // run 内部已把错误画进节点
      });
    } catch (e) { /* 语法/环境异常：节点保留源码样式 */ }
  }
  ...
  if (typeof document !== 'undefined' && document.addEventListener) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      new MutationObserver(moKick).observe(document.body, { childList: true, subtree: true });
      moKick();
    });

    /* === mermaid 相册式预览：点击图表全屏放大，滚轮/双指缩放 + 拖拽平移 + 双击复位 === */
    document.addEventListener('DOMContentLoaded', function () {
      if (document.getElementById('mdmZoomMask')) return;
      var mask = document.createElement('div');
      mask.id = 'mdmZoomMask';
      mask.innerHTML = '<div class="mdm-zoom-stage"></div>' +
        '<button class="mdm-zoom-close" type="button" aria-label="关闭">✕</button>' +
        '<div class="mdm-zoom-tip">滚轮 / 双指缩放 · 拖拽移动 · 双击复位 · Esc 关闭</div>';
      document.body.appendChild(mask);
      var stage = mask.querySelector('.mdm-zoom-stage');
      var svg = null;
      ...
      function open(srcSvg) {
        ...
        svg.removeAttribute('style'); // 清掉 mermaid 内联宽度，交给预览层 CSS 适配
        svg.removeAttribute('class');
        svg.classList.add('mdm-zoom-svg');
        stage.innerHTML = '';
      ...
      mask.querySelector('.mdm-zoom-close').addEventListener('click', close);
      ...
      mask.addEventListener('wheel', function (e) {
        e.preventDefault();
        var ns = Math.min(12, Math.max(0.2, state.s * (e.deltaY < 0 ? 1.15 : 1 / 1.15)));
        var k = ns / state.s;
      ...
      // 拖拽平移 + 双指捏合（pointer events 统一处理鼠标/触摸）
      var ptr = new Map(), lastDist = 0;
      mask.addEventListener('pointerdown', function (e) {
        ptr.set(e.pointerId, { x: e.clientX, y: e.clientY });
        try { mask.setPointerCapture(e.pointerId); } catch (err) {}
        if (ptr.size === 2) {
          var a = Array.from(ptr.values());
      ...
      mask.addEventListener('pointermove', function (e) {
        ...
        if (ptr.size === 1) {
          state.x += e.clientX - p.x; state.y += e.clientY - p.y;
          apply();
        }
        ...
        if (ptr.size === 2) {
          ...
          if (lastDist > 0 && d > 0) {
            var ns = Math.min(12, Math.max(0.2, state.s * (d / lastDist)));
            var k = ns / state.s;
            state.x = mid.x - (mid.x - state.x) * k;
            state.y = mid.y - (mid.y - state.y) * k;
            state.s = ns;
            apply();
          }
          lastDist = d;
      ...
      ['pointerup', 'pointercancel'].forEach(function (ev) {
        mask.addEventListener(ev, function (e) { ptr.delete(e.pointerId); lastDist = 0; });
      });
      ...
      // 事件委托：点击任意已渲染 mermaid 打开预览（动态渲染的也覆盖）
      document.addEventListener('click', function (e) {
        if (e.button !== 0 || !e.target || !e.target.closest) return;
        var el = e.target.closest('.mermaid[data-mermaid-done]');
        if (!el || el.closest('#mdmZoomMask') || !el.querySelector('svg')) return;
