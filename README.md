...
> 在线演示（低门槛试玩版）：http://103.240.199.228:20030 · 演示环境请勿投入生产使用，请勿填写真实隐私信息

---

> ⚠️ **当前为 v7.1-beta**：文件夹系统 / AI 整理 Agent / Windows 式多选为新功能，仍在验收期。上一稳定版 v6.8.1 见 [`stable` 分支](https://github.com/conm599/pixel-notes/tree/stable)。

### 🗂️ v7.x 新特性
- **便签文件夹**：多层嵌套、面包屑导航、递归计数、删除内容上移（绝不丢便签）
- **AI 整理 Agent**：SSE 透明执行（实时看思考/工具调用）；8 种操作（移动/建夹/改名/删除/排序/颜色/置顶）；可先查后做（读便签全文/列文件夹）；预览确认 + 一键撤销（含删除的便签原样复活）
- **Windows 式多选**：长按选中、全页框选（粉色选框）、Ctrl+X/C/V/A、剪贴板独立于选择（可跨文件夹反复粘贴）
- **模糊搜索**：标题/内容多级匹配，结果带路径直达
- **数据备份迁移**（v7.1）：管理页三级页全库导出 JSON；导入为**增量合并**——同邮箱账号数据自动并入（便签归置到「其他账号的数据」文件夹并邮件通知本人），新邮箱原样建号，绝不覆盖现有数据
- **替换块定位加固**（v7.1，protocol v8）：SEARCH 三级匹配全部要求全文唯一命中（重复句式宁可报错不错改）+ 多块隔离（后块不得改写前块刚输出的新文本），根治「没让改的句子自己搬家」类错位

## 🧪 demo 分支说明（公开演示机专用）

> 本分支 ≠ 生产代码。仅用于在公开演示机上跑一个「低门槛试玩版」，**请勿用本分支部署生产**。

- **生产代码**：`main` 分支（完整安全策略；生产站地址不公开，部署后自定）
- **本分支差异**：注册免邮箱验证（用户名+密码直接注册）；登录支持用户名；验证码登录/找回密码入口移除（演示机未配 SMTP）；所有 AI 相关功能未配置不可用；全站演示横幅+进入弹窗
- **隐私提醒**：演示站数据不承诺保留，请勿填写真实邮箱 / API Key / 常用密码
- 生产部署请使用 `main` 分支
...
- **替换块定位双保险**（protocol v8）：SEARCH 三级匹配（精确 / 行尾空白归一 / 全空白折叠）全部要求**全文唯一命中**，重复句式宁可判失败进重试也不错改；多块顺序应用时**块隔离**——后块不得改写前块刚输出的新文本，杜绝「没让改的句子自己搬家」
...
### 🔐 账号体系
- **邮箱唯一登录**（v7.1）：邮箱是唯一登录凭据与账号标识，用户名仅作昵称展示、允许重复
...
│   ├── notes.php        # 便签 CRUD + 分享管理（含文件夹归属）
│   ├── folders.php      # 文件夹树/建/改名/移动/排序/删除（内容上移）
...
├── js/
│   ├── app.js           # 主应用逻辑
│   ├── ai-direct.js     # AI 浏览器直连模块（与服务端协议逐字一致）
│   ├── selection.js     # Windows 式多选模块（框选/剪贴板/快捷键）
│   ├── auth.js          # 登录/注册页逻辑
