...
- **用户自有 Key 三通道**：
  1. 不填自己的代理 → 经**管理员配置的平台透明代理**（CF Worker）转发，服务器永不直连用户目标（内置 SSRF 防护，管理员未配置代理时该通道不可用；每账号每日 500 次防刷）
...
| AI 澄清提问（多轮）：拿不准时先问用户，轮数不限、每轮最多 3 问，回答后继续；澄清轮不扣配额 | `api/ai.php` / `js/ai-direct.js` |
