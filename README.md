...
| 文件 | 版本 | 职责 |
|---|---|---|
| js/app.js | **v134** | 前端总交互：抽屉导航（E 键）、上下文右键菜单、网盘式移动弹窗、快捷键（T//）、搜索弹层、全屏编辑器实时预览、像素输入弹窗、弹窗过渡双模式、AI 设置/编辑流（SSE 解析）、图床联动初始化、保存防竞态、文件夹分享角标即时同步 |
| js/md.js | **v43** | PixelMD 渲染器（表格/任务列表/高亮/上下标/脚注/媒体白名单；AI 图片尺寸 `<img width>` 支持，px 后缀容错；表格横向滚动容器） |
| css/pixel.css | **v105**（全站统一） | 粉色像素风设计系统（含像素图标三态、文件夹分享角标、抽屉/搜索弹层/移动弹窗样式） |
