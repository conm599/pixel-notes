...
> ⚠️ **当前为 v8-beta**：在 v7.5 基础上新增——账号统一与图床联动、域名动态化（第二域名零改码绑定）、配置套件化（/admini 配置中心）、登录 TTL 24H、请求三通道容错、内置四难度教程、全线 GET 化刷新等，仍在验收期。上一稳定版 v6.8.1 见 [`stable` 分支](https://github.com/conm599/pixel-notes/tree/stable)。
...
### 🌐 账号统一与域名动态化（v8-beta）
- **双站共享登录**：便签与图床（配套项目 Pixel Suite）通过父域 Cookie 一次登录双站互通，顶栏/设置菜单一键互跳
- **域名动态化**：Cookie 父域从当前请求 host 自动推导；站间互跳链接支持**配置化**（suite-config 的 `bianqian_host` / `tuchang_host`）与**动态前缀互换**（`bianqian.<域>` ↔ `tuchang.<域>`）——绑定第二域名**零改码**
- **配置套件化**：所有凭证与常量三层通道——环境变量 `PSU_*` > `suite-config.php`（配置中心写）> 代码默认值，**凭证不入码不入仓**

### 🖥️ /admini/ 配置中心（v8-beta）
- **安装向导**：全新部署打开 `/admini/` 按表单填写 DB/域名/密钥，注册账号即双站管理员，完成后锁定
- **配置面板**：22 项硬编码全量收编（DB/邀请码/密令/配额/TTS/站点域名），改完立即生效无需重启；敏感字段留空不改；环境变量可单项覆盖
- **密令门禁**：管理后台（adminws）密码 + 图形验证码 + IP 限频 + CSRF + 防时序比较 + 会话固定防护

### 🔐 登录与安全强化（v8-beta）
- **登录 TTL 24H**：Cookie lifetime + 服务端 gc_maxlifetime 同值 + 已登录滑动续期（活跃永不掉线）
- **全站 GET 只读白名单**：list/folder_list 等只读操作走 GET（规避部分线路 POST 响应丢包），写操作仍强制 POST + CSRF
- **三通道请求容错**：fetch → 重试 → XHR 换通道；响应丢失时 UI 按服务器实际状态刷新，不误报
- **请求级防重锁**：同参数请求进行中忽略重复（双击/重试双发根除）

### 📖 内置教程（v8-beta）
- **四难度新手教程**（低/中/高/大师）内置应用内弹窗，Markdown 全语法渲染（教程本身即 MD 能力演示）
- 内容与仓库 `docs/` 教程文档同源（构建脚本生成），改文档重跑脚本即更新

### ⚡ 稳定性与性能修缮（v8-beta）
- 批量上传队列卡死修复（漏 return Promise）
- 代码块长行自动折行（手机卡片溢出根治）
- 首屏摘要瘦身 + 零请求切换文件夹 + 排序持久化（notesOrder 数组方案）
- 新手教程弹窗/文件夹创建/删除/改名的网络容错与状态同步全链路修缮
